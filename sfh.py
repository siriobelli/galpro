...
def identify_sfh_type(pars):
    """
    Identify type of SFH from the dictionary of parameters 
    """
    ...
    # if you made it this far, things aren't good
    raise NotImplementedError('Supported SFHs are only nonparametric, ssp, tau, deltau')
    
    
def create_sfh(input):
    """
    Create the appropriate type of SFH and return it.
    The input is either a model or a model.params dictionary
    """
    ...
    sfh_type = identify_sfh_type(pars)
    if sfh_type == 'nonparametric':
        return NonparametricSFH(**pars)
    if sfh_type == 'tau':
        return ExponentialSFH(**pars)
    if sfh_type == 'deltau':
        return DelayedExponentialSFH(**pars)
...
class NonparametricSFH(SFH):
    """
    Class describing a non-parametric star formation history in prospector
    Note that an object of this class represents a specific instance of the 
    model and has zero free parameters 
    """
    
    
    def __init__(self, agebins, logsfr_ratios, logmass, **kwargs):
        """
        ...
        self.logmass = np.float(logmass)
        self.tage = 10**np.max(self.logagebins)
         
        # # store relevant parameters
        # self.logagebins = model.params['agebins']
        # self.logsfr_ratios = model.params['logsfr_ratios']
        # self.logmass = np.float(model.params['logmass'])
        # self.tage = 10**np.max(self.logagebins)

    
    def bin_mass(self):
        """
        Return mass formed (in Msun) in each agebin
        """
        
        return transforms.logsfr_ratios_to_masses(logmass=self.logmass, logsfr_ratios=self.logsfr_ratios, agebins=self.logagebins)
    ...
    def mass_formed(self, t):
        ...
        mass_formed_after = np.cumsum( np.concatenate(([0.0], self.bin_mass())) )
        mass_formed_before = np.sum(self.bin_mass()) - mass_formed_after
